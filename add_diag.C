...
void add_diag(Bool_t useTightCuts=false,
              Int_t whichClass=0) {
  gROOT->Reset();
  ...
  // -- sorry, but this tree has a weird structure... there is a branch
  ...
  for(int s=0; s<NSETS; s++) {
    ...
    while(key=(TKey*)next()) {
      ...
      // overlap_matrices key sector
        ...
        diag_file[s]->cd();
      } // eo overlap_matrices sector


      // rdists & threshtr sector
            ...
            if(!strcmp("pt",rdist_types[rtp].Data())) {
              if(rdist[rc][rtp][rint][rtg][s]->GetEntries()>0) {
                ...
                if(curr_fit!=NULL) {
                  ...
                  else thresh = rdist[rc][rtp][rint][rtg][s]->GetBinCenter(binn);
      ...
      // generic TObjArrays sector (for kinematic correlations plots and mass dists)
        ...
        if(strcmp(classname_tmp,"mass")) {
          ...
          if(cc!=whichClass) continue;  // only do one class at a time for kinematic correlations (memory leak "patch")
      ...
      }; // eo generic TObjArray sector

    }; // eo key loop
  }; // eo diagset file loop
