...
void print_diag2(Bool_t useTightCuts=false,
                 Int_t whichClass==0) {
  ...
  TString rmstr = ".! rm -rv pdf_kincorr"+tightstr;
          ...
          if(gROOT->GetClass(plot_key->GetClassName())->InheritsFrom("TDirectory")) {
            ...
            while(trig_key=(TKey*)trig_it()) {
              if(gROOT->GetClass(trig_key->GetClassName())->InheritsFrom("TObjArray")) {
                ...
                pdf_name = "pdf_kincorr"+tightstr+"/"+curr_dir_name+"/"+TString(trig_key->GetName())+".pdf";
                mkdir_str = ".! mkdir -pv pdf_kincorr"+tightstr+"/"+curr_dir_name;
                //printf("execute: %s\n",mkdir_str.Data());
