...
void KinVarVsRun(Bool_t useTightCuts=false) {
      ...
      for(Int_t t=0; t<N_TRIG; t++) {
        ...
        for(Int_t r=0; r<nruns; r++) {
          ...
          if(r==0) {
            ...
            kvr_t[c][k][t] = Form("%s %s %s vs. internal run index;internal run index;%s",
                                  (LT->Name(t)).Data(),
                                  ev->Title(c),
                                  kin_title[k].Data(),
                                  kin_title[k].Data());
            ...
            if(k==0 || k==1) {
              thresh_gr[c][k][t] = new TGraph();
              thresh_gr[c][k][t]->SetLineWidth(3);
              thresh_gr[c][k][t]->SetLineColor(kBlack);
            };
          };
  ...
  // TProfiles
  TProfile * kvr_pfx[N_CLASS][N_KIN][N_TRIG];
  for(Int_t c=0; c<N_CLASS; c++) {
    for(Int_t k=0; k<N_KIN; k++) {
      for(Int_t t=0; t<N_TRIG; t++) {
        if(kvr[c][k][t]!=NULL) {
          kvr_pfx[c][k][t] = kvr[c][k][t]->ProfileX();
          kvr_pfx[c][k][t]->SetMarkerStyle(kFullCircle);
          kvr_pfx[c][k][t]->SetMarkerColor(kMagenta);
        };
      };
    };
  };
  ...
  TCanvas * kvr_canv[N_CLASS][N_KIN][N_TRIG];
  for(Int_t c=0; c<N_CLASS; c++) {
    for(Int_t k=0; k<N_KIN; k++) {
      for(Int_t t=0; t<N_TRIG; t++) {
        kvr_n[c][k][t] = kvr_n[c][k][t]+"_canv";
        kvr_canv[c][k][t] = new TCanvas(kvr_n[c][k][t].Data(),kvr_n[c][k][t].Data(),800,800);
        if(kvr[c][k][t]!=NULL) {
          kvr[c][k][t]->Draw("colz");
          kvr_pfx[c][k][t]->Draw("histpsame");
          if(k==0 || k==1) {
            if(thresh_gr[c][k][t]->GetN()>0) {
              thresh_gr[c][k][t]->Draw("L");
            };
          };
          kvr_canv[c][k][t]->Write();
        };
      };
    };
  };
