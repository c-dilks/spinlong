...
void Purity(Int_t eta_bin = 0,
  ...
  // load libraries
  Bool_t useLibs = false; // if true, needs full environment and libs
  if(useLibs) {
  ...
  if(useLibs) {
    eta_lb = RD->env->EtaDiv(eta_bin);
    eta_ub = RD->env->EtaDiv(eta_bin+1);
    en_lb = RD->env->EnDiv(en_bin);
    en_ub = RD->env->EnDiv(en_bin+1);
    pt_lb = RD->env->PtDiv(pt_bin);
    pt_ub = RD->env->PtDiv(pt_bin+1);
  } else {
    eta_lb = -1.0;
    eta_ub = -1.0;
    en_lb = -1.0;
  ...
  // output mass cuts
  TString massset_dir;
  if(useLibs) massset_dir = TString(RD->env->massset_dir);
  else {
    TRegexp re("\/.*$");
    massset_dir = infile_n;
    massset_dir(re) = "";
  };
  TString statsfile = Form("%s/stat_g%d_e%d_p%d.dat",massset_dir.Data(),eta_bin,en_bin,pt_bin);
  ...
  // print canvas
  TString pdffile = Form("%s/canv_g%d_e%d_p%d.pdf",massset_dir.Data(),eta_bin,en_bin,pt_bin);
...
TCanvas * ExeFit(TH1D * hh, 
  ...
  cheb[5]="(16*TMath::Power(mass,5)-20*TMath::Power(mass,3)+5*mass)";
  if(order>0) {
    // zeroth term has coefficient constrained by other coefficients to
    // ensure that the background is zero at zero mass
    if(order==2 || order==3) 
      formu_bg = "b1*mass+2*TMath::Power(mass,2)";
    else if(order==4 || order==5) 
      formu_bg = "1+b1*mass+(b4+1)*(2*TMath::Power(mass,2)-1)";
    else {
      fprintf(stderr,"error: bad polynomial order\n");
      return;
    };
    if(order>2) {
      for(int co=3; co<=order; co++) {
    ...
    switch(order) {
      ...
      case 3:
       RooGenericPdf bg_func("background","bg_func",
                             formu_bg.Data(),RooArgList(mass,b1,b3));
       break;
      case 4:
       RooGenericPdf bg_func("background","bg_func",
                             formu_bg.Data(),RooArgList(mass,b1,b3,b4));
       break;
      case 5:
       RooGenericPdf bg_func("background","bg_func",
                             formu_bg.Data(),RooArgList(mass,b1,b3,b4,b5));
       break;
      default:
       return;
    };
  ...
  */
  ...
  switch(binhash) {
    case 0x000:
      ...
      RooRealVar b1("b1","b1", b_lb, b_ub);
      RooRealVar b2("b2","b2", 1, 0.99, 1.01);
      RooRealVar b3("b3","b3", b_lb, b_ub);
      RooRealVar b4("b4","b4", b_lb, b_ub);
      RooRealVar b5("b5","b5", b_lb, b_ub);
      break;
    default:
      ...
      RooRealVar b1("b1","b1", 6.9, b_lb, b_ub);
      RooRealVar b2("b2","b2", 1, 0.99, 1.01);
      RooRealVar b3("b3","b3", 28.6, b_lb, b_ub);
      RooRealVar b4("b4","b4", 9.6, b_lb, b_ub);
      RooRealVar b5("b5","b5", b_lb, b_ub);
  };
  if(order>0) {
    switch(order) {
      case 2:
       RooChebychev bg_func("background","bg_func",mass,RooArgSet(b1,b2));
       break;
      case 3:
       RooChebychev bg_func("background","bg_func",mass,RooArgSet(b1,b2,b3));
       break;
      case 4:
       RooChebychev bg_func("background","bg_func",mass,RooArgSet(b1,b2,b3,b4));
       break;
      case 5:
       RooChebychev bg_func("background","bg_func",mass,RooArgSet(b1,b2,b3,b4,b5));
       break;
      default:
       return;
    };
  ...
  if(order>0) {
    switch(order) {
      case 2:
       formu_bg = "b1*mass+b2*TMath::Power(mass,2)";
       RooGenericPdf bg_func("background","bg_func",
                             formu_bg.Data(),RooArgList(mass,b1,b2));
       break;
      case 3:
       formu_bg = "b1*mass+b2*TMath::Power(mass,2)+b3*TMath::Power(mass,3)";
       RooGenericPdf bg_func("background","bg_func",
                             formu_bg.Data(),RooArgList(mass,b1,b2,b3));
       break;
      case 4:
       formu_bg = "b1*mass+b2*TMath::Power(mass,2)+b3*TMath::Power(mass,3)+b4*TMath::Power(mass,4)";
       RooGenericPdf bg_func("background","bg_func",
                             formu_bg.Data(),RooArgList(mass,b1,b2,b3,b4));
       break;
      case 5:
       formu_bg = "b1*mass+b2*TMath::Power(mass,2)+b3*TMath::Power(mass,3)+b4*TMath::Power(mass,4)+b5*TMath::Power(mass,5)";
       RooGenericPdf bg_func("background","bg_func",
                             formu_bg.Data(),RooArgList(mass,b1,b2,b3,b4,b5));
       break;
      default:
       return;
    };
